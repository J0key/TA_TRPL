    ...
    <main class="w-full ml-[330px] mr-[50px] min-h-screen flex flex-col">
        ...
        <div class="flex flex-row items-center w-full justify-between mt-8">
            <div class="text-[#404D61] mt-5">
                ...
                <p id="jml_TA" class="font-regular text-xl">0 results found</p>
            </div>
        ...
        <div class="flex flex-row w-full mt-8">
            <button onclick="toAjukanJudul()" class=" bg-semi_dark_green px-6 py-2 text-white rounded-xl">
                <p>Ajukan Tugas Akhir</p>
            </button>
        </div>

        {{-- table start --}}
        <div class="rounded-lg border border-gray-200 shadow-md mt-4 w-full">
            <table class="w-full border-collapse bg-white text-left text-sm text-gray-500 overflow-x-scroll">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col" class="px-6 py-4 font-medium text-gray-900" style="min-width: 200px;">Judul
                            Proyek</th>
                        <th scope="col" class="px-6 py-4 font-medium text-gray-900" style="min-width: 180px;">
                            Penanggung Jawab</th>
                        <th scope="col" class="px-6 py-4 font-medium text-gray-900" style="min-width: 300px;">
                            Deskripsi</th>
                        <th scope="col" class="px-6 py-4 font-medium text-gray-900" style="min-width: 120px;">Tools
                        </th>
                        <th scope="col" class="px-6 py-4 font-medium text-gray-900" style="min-width: 150px;">Tanggal
                            Upload</th>
                        <th scope="col" class="px-6 py-4 font-medium text-gray-900" style="min-width: 100px;">Action
                        </th>
                    </tr>
                </thead>
...
<script>
    function toAjukanJudul() {
        window.location.href = "{{ route('mahasiswa.pengajuan') }}";
    }
    ...
    async function fetchProjects(page = 1, filterName = '') {
        try {
            response = await fetch(`http://127.0.0.1:8001/api/project?status=not%20taken%20yet&page=${page}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
    ...
    function displayProjects(projects) {
        ...
        projects.forEach(project => {
            ...
            td2.textContent = (project.Dosen_1.user["first_name"] + " " + project.Dosen_1.user["last_name"]);
            ...
            td5.textContent = project['Created_at'].split("T")[0];
    ...
    function toProjectDetails(id) {
        const url = `{{ route('mahasiswa.pengajuanSudahAda') }}?id=${id}`;
